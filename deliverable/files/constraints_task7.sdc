...
# Clock Period Calculation: T = 1 / 250 MHz = 4.0 ns

################################################################
# 1, 2, 3, 4) Clock Definition (clk) and Characteristics
################################################################

# 1) Create clock 'clk' (250 MHz, 50% duty) πάνω στην πόρτα wb_clk_i
set clk_port [get_ports wb_clk_i]
create_clock -name clk -period 4.0 -waveform {0 2.0} $clk_port
